...
class Biz::PayRoute::BjrcbPayRoute < Biz::PayRoute::PayRouteBase
  ...
  def initialize(merchant, channel_type)
    ...
    unless [1,0].include?(channel_type.to_i)
      raise "你传入的是channel_type：#{channel_type}不符合要求，必须是1=>T1或0=>D0."
    end
  ...
  def send_wechat_offline
    ...
    create_route_request(hash) # Biz::PayRoute::PayRouteBase => @request_js
    data = send_request # @request_js Biz::PayRoute::PayRouteBase
    log_js = {
