...
set :shared_files, fetch(:shared_files, []).push('config/database.yml', 'config/mongoid.yml','config/secrets.yml', 'config/puma.rb')
...
task :deploy do
  ...
  deploy do
    ...
    on :launch do
      ...
      #   invoke :environment
      #   command %{ RAILS_ENV=production rails import:locations }
      # end
      in_path("/home/rb") do
        invoke :environment
